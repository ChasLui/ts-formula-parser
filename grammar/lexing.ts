import { createToken, Lexer, IToken, TokenType, ILexingError } from 'chevrotain';
import FormulaError from '../formulas/error';
import type { ErrorDetails } from '../types';

// Token vocabulary type definition
...
interface LexerInterface {
    ...
    lex: (inputText: string) => LexingResult;
}

const lexer: LexerInterface = {
    tokenVocabulary: tokenVocabulary,

    lex: function (inputText: string): LexingResult {
        const lexingResult = SelectLexer.tokenize(inputText)

        if (lexingResult.errors.length > 0) {
            const error = lexingResult.errors[0];
            const line = error.line || 1, column = error.column || 1;
            let msg = '\n' + inputText.split('\n')[line - 1] + '\n';
            msg += Array(column - 1).fill(' ').join('') + '^\n';
            const errorMessage = msg + `Error at position ${line}:${column}\n` + error.message;
            const errorWithLocation = { ...error, errorLocation: {line, column} };
            throw FormulaError.ERROR(errorMessage, errorWithLocation as ErrorDetails);
        }
