...
/**
 * Type enumeration for formula parameters
 */
const Types = {
    NUMBER: 0,
    ARRAY: 1,
...
/**
 * Type alias for type values
 */
export type TypeValue = typeof Types[keyof typeof Types];

/**
 * Pre-calculated factorials array
 */
...
/**
 * Reversed types map for debugging/display purposes
 */
const ReversedTypes: Record<number, string> = {};
Object.keys(Types).forEach((key) => {
    ReversedTypes[Types[key as keyof typeof Types]] = key;
});

/**
 * Interface for parameters with metadata
 */
...
/**
 * Interface for parameter info used in hooks
 */
interface ParamInfo {
    isLiteral: boolean;
    isCellRef: boolean;
...
/**
 * Criteria parsing result interface
 */
interface CriteriaResult {
    op: string;
    value: string | number | boolean | RegExp | FormulaError;
    match?: boolean;
}

/**
 * Context interface for function execution
 */
interface ExecutionContext {
    utils: {
        extractRefValue(arg: any): { val: any; isArray: boolean };
    };
}
...
class FormulaHelpers {
    ...
    private readonly type2Number: Record<string, number> = {
        number: Types.NUMBER,
    ...
    /**
     * Check if function result is valid, handle special cases like NaN and infinity
     * @param result - The result to check
     * @returns The validated result or error
     */
    ...
    /**
     * Flatten a nested array deeply
     * @param arr1 - Array to flatten
    ...
    /**
     * Accept and convert a value to number
     ...
     * @returns The converted number or FormulaError
     */
    acceptNumber(obj: any, isArray: boolean = true, allowBoolean: boolean = true): any {
        // check error
        ...
        // "123" -> 123
            ...
            throw new Error('Unknown type in FormulaHelpers.acceptNumber')
        }
        return number;
    }
     ...
     * @param params - Parameters that need to flatten
     * @param valueType - The type each item should be, null if allows any type
     * @param allowUnion - Allow union, e.g. (A1:C1, E4:F3)
     * @param hook - Invoked after parsing each item
     * @param defValue - The value if a param is omitted
     * @param minSize - The minimum size of the parameters
     */
    flattenParams(
        params: any[], 
        valueType: TypeValue | null, 
        allowUnion: boolean, 
        hook: (param: any, info: ParamInfo) => void, 
        defValue: any = null, 
        minSize: number = 1
    ): void {
        if (params.length < minSize)
            throw FormulaError.ARG_MISSING([valueType?.toString() || 'unknown']);
        if (defValue == null) {
        ...
        params.forEach(param => {
            ...
            const info: ParamInfo = {isLiteral, isCellRef, isRangeRef, isArray, isUnion};
     ...
     * @param flat - If the array should be flattened, only applicable when type is ARRAY
     * @param allowSingleValue - If pack single value into 2d array, only applicable when type is ARRAY
     * @returns The validated and converted parameter
     */
    accept(
        param: any, 
        type: TypeValue | TypeValue[] | null = null, 
        defValue?: any, 
        flat: boolean = true, 
        ...
        if (param == null && defValue === undefined) {
            throw FormulaError.ARG_MISSING([type?.toString() || 'unknown']);
        } else if (param == null)
            return defValue;
     ...
     * @param variable - The variable to check type for
    ...
    /**
     * Check if parameter is a range reference
     */
    ...
    /**
     * Check if parameter is a cell reference
     */
    ...
    /**
     * Helper function for SUMIF, AVERAGEIF, etc.
     * @param context - Execution context
     * @param range1 - First range
     * @param range2 - Second range
     * @returns Array containing processed ranges
     */
    retrieveRanges(context: ExecutionContext, range1: any, range2: any): [any, any] {
        // process args
    ...
    /**
     * Retrieve argument value from context
     * @param context - Execution context
     * @param arg - Argument to retrieve
     * @returns Retrieved argument with metadata
     */
    retrieveArg(context: ExecutionContext, arg: any): ParamWithMeta {
        if (arg === null)
...
/**
 * Wildcard utilities for string matching
 */
const WildCard = {
    /**
     ...
     * @param lookupText - Pattern with wildcards
     * @param flags - RegExp flags
     * @returns RegExp for matching
     */
    toRegex: (lookupText: string, flags?: string): RegExp => {
        return new RegExp(lookupText.replace(/[.+^${}()|[\]\\]/g, '\\$&') // escape the special char for js regex
            .replace(/([^~]??)[?]/g, '$1.') // ? => .
            .replace(/([^~]??)[*]/g, '$1.*') // * => .*
...
/**
 * Criteria parsing utilities
 */
const Criteria = {
    /**
     * Parse criteria, support comparison and wild card match.
     * @param criteria - Criteria to parse
     * @returns The parsed criteria object
     */
    parse: (criteria: any): any => {
        ...
        if (type === "string") {
            ...
            if (res) {
                ...
                let op = res[1], value: string | number | boolean | FormulaError;
                ...
                if (isNaN(Number(res[2]))) {
                    ...
                    if (upper === 'TRUE' || upper === 'FALSE') {
                        ...
                        if (WildCard.isWildCard(value)) {
                            return {op: 'wc', value: WildCard.toRegex(value as string), match: op === '='}
                        }
                    }
                } else {
                    // number
            ...
            throw new Error(`Criteria.parse: type ${typeof criteria} not support`)
        }
    }
};

/**
 * Address utilities for cell/range operations
 */
const Address = {
    /**
     * Convert column number to Excel column name (1 -> A, 26 -> Z, 27 -> AA)
     * @param number - Column number (1-based)
     * @returns Column name
     */
    ...
    /**
     * Convert Excel column name to number (A -> 1, Z -> 26, AA -> 27)
     ...
     * @param range1 - First range to match dimensions with
     * @param range2 - Second range to extend
    ...
    extend: (range1: any, range2?: any): any => {
        ...
        } else throw new Error('Address.extend should not reach here.');
...
// Export the helper instance and utilities
export { 
    H as FormulaHelpers, 
    Types, 
    ReversedTypes, 
