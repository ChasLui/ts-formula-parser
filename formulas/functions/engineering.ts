...
const EngineeringFunctions = {
    ...
    IMPRODUCT: (...params) => {
        ...
        H.flattenParams(params, null, false, (item) => {
            if (i === 0) {
                ...
                if (unit1 !== unit2)
                    throw FormulaError.VALUE;
                result = EngineeringFunctions.COMPLEX(a * c - b * d, a * d + b * c, unit1);
            }
            i++;
        }, 1);
        return result;
    ...
    IMTAN: (iNumber) => {
        const {unit} = parseIM(iNumber);
        return EngineeringFunctions.IMDIV(EngineeringFunctions.IMSIN(iNumber), EngineeringFunctions.IMCOS(iNumber));
    },
