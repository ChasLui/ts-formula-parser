...
describe('Dependency parser', () => {
    it('parse SUM(1,)', () => {
        const actual: (CellRef | RangeRef)[] = depParser.parse('SUM(1,)', position);
        expect(actual).to.deep.eq([]);
    });

    it('should parse single cell', () => {
        let actual: (CellRef | RangeRef)[] = depParser.parse('A1', position);
    ...
    it('should parse the same cell/range once', () => {
        let actual: (CellRef | RangeRef)[] = depParser.parse('A1+A1+A1', position);
        expect(actual).to.deep.eq([position]);
    ...
    it('should parse ranges', () => {
        let actual: (CellRef | RangeRef)[] = depParser.parse('A1:C3', position);
    ...
    it('should parse variable', function () {
        const actual: (CellRef | RangeRef)[] = depParser.parse('aaaa', position);
    ...
    it('should parse basic formulas', function () {
        ...
        // data types
        let actual: (CellRef | RangeRef)[] = depParser.parse('TRUE+A1+#VALUE!+{1}', position);
    ...
    it('should parse complex formula', () => {
        const actual: (CellRef | RangeRef)[] = depParser.parse('IF(MONTH($K$1)<>MONTH($K$1-(WEEKDAY($K$1,1)-(start_day-1))-IF((WEEKDAY($K$1,1)-(start_day-1))<=0,7,0)+(ROW(O5)-ROW($K$3))*7+(COLUMN(O5)-COLUMN($K$3)+1)),"",$K$1-(WEEKDAY($K$1,1)-(start_day-1))-IF((WEEKDAY($K$1,1)-(start_day-1))<=0,7,0)+(ROW(O5)-ROW($K$3))*7+(COLUMN(O5)-COLUMN($K$3)+1))', position);
        expect(actual).to.deep.eq([
            {
                "col": 11,
                "row": 1,
