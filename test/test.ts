...
import './grammar/test';
import './grammar/errors';
import './grammar/collection';
import './grammar/depParser';
import './formulas/index';
...
describe('Parsing Formulas 2', () => {
    ...
    beforeAll(async () => {
        return new Promise<void>((resolve, reject) => {
            fs.readFile('./test/formulas2.json', (err: NodeJS.ErrnoException | null, data: Buffer) => {
                if (err) {
                    reject(err);
                    return;
                }
