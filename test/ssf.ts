...
describe('SSF (Spreadsheet String Format)', () => {
    describe('Basic formatting functionality', () => {
        ...
        it('should correctly format scientific notation', () => {
            const result: string = SSF.format('0.00E+00', 1234);
    ...
    describe('Date formatting', () => {
        it('should correctly format date objects', () => {
            const testDate: Date = new Date(2023, 0, 15, 14, 30, 0); // 2023-01-15 14:30:00

            // Test basic date format
            const result1: string = SSF.format('m/d/yy', testDate);
            expect(result1).to.match(/1\/15\/23/);

            // Test year-month-day format
            const result2: string = SSF.format('yyyy-mm-dd', testDate);
            expect(result2).to.match(/2023-01-15/);
        });

        it('should handle date-related built-in formats', () => {
            const testDate: Date = new Date(2023, 0, 15);
            const result: string = SSF.format(14, testDate); // 14 is a date format index
        ...
        it('should support parse_date_code', () => {
            ...
            const parsedResult: DateParseResult | null = SSF.parse_date_code(dateCode);
    ...
    describe('Table management', () => {
        it('should get format table', () => {
            const table: SSFTable = SSF.get_table();
        ...
        it('should load custom format', () => {
            const originalTable: SSFTable = SSF.get_table();
            const customFormat = '0.000';
            const index: number = SSF.load(customFormat, 100);

            expect(index).to.equal(100);
            const newTable: SSFTable = SSF.get_table();
    ...
    describe('Special formats and edge cases', () => {
        it('should handle fraction format', () => {
            const result: string = SSF.format('# ?/?', 0.5);
            expect(result).to.match(/1\/2/);
        });

        it('should handle large numbers', () => {
            const largeNum = 1e10;
            const result: string = SSF.format('0.00E+00', largeNum);
            expect(result).to.be.a('string');
    ...
    describe('Options and configuration', () => {
        it('should support date1904 option', () => {
            const testDate: Date = new Date(2023, 0, 15);
            const result1: string = SSF.format('m/d/yy', testDate, {date1904: false});
            const result2: string = SSF.format('m/d/yy', testDate, {date1904: true});
        ...
        it('should support custom table option', () => {
            const customTable: SSFTable = {100: '0.000'};
            const result: string = SSF.format(100, 1.23, {table: customTable});
        ...
        it('should support custom dateNF option', () => {
            ...
            const result1: string = SSF.format('m/d/yy', testDate, options);
            expect(result1).to.match(/2023-01-15/);

            // Test format index 14 using custom dateNF
            const result2: string = SSF.format(14, testDate, options);
            expect(result2).to.match(/2023-01-15/);
        });
    });
    ...
    describe('Error handling and edge cases', () => {
        it('should handle valid format indices', () => {
            ...
            const result1: string = SSF.format(0, 123);
            expect(result1).to.be.a('string');

            // Test string format
            const result2: string = SSF.format('0', 123);
    ...
    describe('Complex format patterns', () => {
        it('should handle conditional formatting', () => {
            ...
            const result1: string = SSF.format(complexFormat, 1500);
            expect(result1).to.be.a('string');

            const result2: string = SSF.format(complexFormat, -500);
            expect(result2).to.be.a('string');

            const result3: string = SSF.format(complexFormat, 0);
            expect(result3).to.be.a('string');
        });

        it('should handle color codes', () => {
            const colorFormat = '[Red]0.00';
            const result: string = SSF.format(colorFormat, 123.45);
            expect(result).to.be.a('string');
        ...
        it('should handle fill characters', () => {
            const fillFormat = '*-0';
            const result: string = SSF.format(fillFormat, 123);
            expect(result).to.be.a('string');
        });
    });
    ...
    describe('Additional format tests', () => {
        it('should handle time formats', () => {
            const testDate: Date = new Date(2023, 0, 15, 14, 30, 45);

            // Time format tests
            const timeResult1: string = SSF.format('h:mm', testDate);
            expect(timeResult1).to.be.a('string');
            expect(timeResult1).to.match(/\d{1,2}:\d{2}/);

            const timeResult2: string = SSF.format('h:mm:ss AM/PM', testDate);
        ...
        it('should handle currency formats', () => {
            const currencyFormats: string[] = [
                '"$"#,##0.00_);("$"#,##0.00)',
                '$#,##0.00',
                '$#,##0_);($#,##0)'
            ];

            currencyFormats.forEach((fmt: string) => {
                const result: string = SSF.format(fmt, 1234.56);
                expect(result).to.be.a('string');
                expect(result.length).to.be.greaterThan(0);
            });
        ...
        it('should handle different date format patterns', () => {
            const testDate: Date = new Date(2023, 11, 25, 15, 30, 45); // 2023-12-25 15:30:45
            ...
            dateFormats.forEach((fmt: string) => {
                const result: string = SSF.format(fmt, testDate);
                expect(result).to.be.a('string');
                expect(result.length).to.be.greaterThan(0);
            });
        ...
        it('should handle accounting formats', () => {
            const accountingFormats: string[] = [
                '_("$"* #,##0.00_);_("$"* (#,##0.00);_("$"* "-"??_);_(@_)',
                '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)'
            ];

            accountingFormats.forEach((fmt: string) => {
                const result: string = SSF.format(fmt, 1234.56);
                expect(result).to.be.a('string');
            });
        });
        ...
        it('should handle month and weekday formats', () => {
            const testDate: Date = new Date(2023, 0, 15); // January 15, 2023, Sunday

            const monthFormats: string[] = ['m', 'mm', 'mmm', 'mmmm'];
            monthFormats.forEach((fmt: string) => {
                const result: string = SSF.format(fmt, testDate);
                expect(result).to.be.a('string');
                expect(result.length).to.be.greaterThan(0);
            });

            const dayFormats: string[] = ['d', 'dd', 'ddd', 'dddd'];
            dayFormats.forEach((fmt: string) => {
                const result: string = SSF.format(fmt, testDate);
                expect(result).to.be.a('string');
                expect(result.length).to.be.greaterThan(0);
            });
    ...
    describe('Advanced formatting and edge cases', () => {
        ...
        it('should handle absolute time formats', () => {
            const testDate: Date = new Date(2023, 0, 15, 25, 70, 80); // Test with overflow values
        ...
        it('should handle seconds with microseconds', () => {
            const testDate: Date = new Date(2023, 0, 15, 14, 30, 45, 123);
        ...
        it('should handle 1904 date system', () => {
            const dateCode = 1000;
            const result1904: DateParseResult | null = SSF.parse_date_code(dateCode, {date1904: true});
            const resultNormal: DateParseResult | null = SSF.parse_date_code(dateCode, {date1904: false});
        ...
        it('should handle write_date with extreme values', () => {
            ...
            const extremeDate: DateParseResult | null = SSF.parse_date_code(1000000); // Very high date code
            if (extremeDate) {
        ...
        it('should handle additional format edge cases for better coverage', () => {
            ...
            const testDate: Date = new Date(2023, 0, 15, 23, 59, 59, 999);
        ...
        it('should test format scenarios to maximize branch coverage', () => {
            ...
            const testDate: Date = new Date(2023, 0, 15, 1, 1, 1, 1);
        ...
        it('should test write_date edge cases for complete branch coverage', () => {
            const testDate: Date = new Date(2023, 0, 15, 0, 0, 0, 0);
            
            // Test era format 'e'
            expect(SSF.format('e', testDate)).to.be.a('string');
            
            // Test AM/PM with different hours
            const morningDate: Date = new Date(2023, 0, 15, 9, 0, 0);
            const eveningDate: Date = new Date(2023, 0, 15, 21, 0, 0);
            ...
            const timeWithMicros: DateParseResult | null = SSF.parse_date_code(44927.999999);
            if (timeWithMicros) {
